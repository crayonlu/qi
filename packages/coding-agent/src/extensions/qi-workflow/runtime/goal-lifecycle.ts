// @ts-nocheck
/**
 * Mature pi-goal lifecycle hosted in Qi. Qi dashboard projects ActiveGoal for UI;
 * GoalRuntime remains source of truth for budget/usage/persistence/continuation.
 * Cross-extension Goal RPC is intentionally excluded (Qi owns command surface).
 */
...
function projectActiveGoal(active: ActiveGoal | undefined, previous?: Goal | null): Goal | null {
...
function syncRuntimeGoalToQi(runtime: GoalRuntime): void {
	workflowController.apply((state: QiWorkflowState) => {
		const projected = projectActiveGoal(runtime.activeGoal, state.goal);
		return { ok: true as const, value: projected, state: { ...state, goal: projected } };
	});
}

function installQiProjection(runtime: GoalRuntime): void {
	const persist = runtime.persistGoal.bind(runtime);
	runtime.persistGoal = (goal: ActiveGoal) => {
		persist(goal);
		syncRuntimeGoalToQi(runtime);
	};
	const clear = runtime.clearActiveGoal.bind(runtime);
	runtime.clearActiveGoal = (ctx: StatusContext, reason = "goal cleared") => {
		clear(ctx, reason);
		syncRuntimeGoalToQi(runtime);
	};
	const updateStatus = runtime.updateStatus.bind(runtime);
	runtime.updateStatus = (ctx: StatusContext, goal: ActiveGoal) => {
		updateStatus(ctx, goal);
		syncRuntimeGoalToQi(runtime);
	};
}

import { currentTokenTotal } from "../vendor/goal/accounting.ts";
...
import { DEFAULT_GOAL_SETTINGS, readGoalSettings } from "../vendor/goal/settings.ts";
...
export function attachGoalLifecycle(pi: ExtensionAPI, options: GoalOptions = {}): GoalRuntime {
	const runtime = new GoalRuntime(pi);
	const commands = new GoalCommandController(runtime);
	goalRuntimeSingleton = runtime;
	goalCommandsSingleton = commands;
	installQiProjection(runtime);
	...
	const sendOwnedGoalPrompt = (_pi: ExtensionAPI, ctx: StatusContext, goalId: string, prompt: string) =>
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		if (runtime.activeGoal) {
			...
			if (startRestoredQueuedGoal) {
				...
				const sent = await sendOwnedGoalPrompt(runtime.pi, ctx, restoredGoal.id, buildGoalPrompt(restoredGoal));
...
let goalRuntimeSingleton: GoalRuntime | undefined;
let goalCommandsSingleton: GoalCommandController | undefined;

export function getGoalRuntime(): GoalRuntime | undefined {
	return goalRuntimeSingleton;
}

export function getGoalCommands(): GoalCommandController | undefined {
	return goalCommandsSingleton;
}
