...
import { spawnCommand } from "./utils/command-executor.ts";
import { isProcessGroupAlive, killProcessGroup } from "./utils/index.ts";
...
export class ProcessManager {
	...
	constructor(options?: ProcessManagerOptions) {
		...
		this.getConfiguredShellPath = options?.getConfiguredShellPath ?? (() => undefined);
	}
	...
	start(name: string, command: string, cwd: string, options?: StartOptions): ProcessInfo {
		...
		child.on("close", (code, signal) => {
			...
			// On Windows, taskkill terminates without a POSIX signal so `signal`
			// is always null. Fall back to `lastSignalSent` to distinguish a
			// deliberate kill from a genuine crash.
			if (signal || managed.lastSignalSent) {
				this.transition(managed, "killed");
			} else {
				this.transition(managed, "exited");
			}
	...
	getOutput(id: string, tailLines = 100): { stdout: string[]; stderr: string[]; status: string } | null {
		const managed = this.processes.get(id);
	...
	getCombinedOutput(id: string, tailLines = 100): { type: "stdout" | "stderr"; text: string }[] | null {
		const managed = this.processes.get(id);
		if (!managed) return null;
	...
	getLogFiles(id: string): { stdoutFile: string; stderrFile: string; combinedFile: string } | null {
		const managed = this.processes.get(id);
		if (!managed) return null;
		return {
	...
	private resolveLogWatches(input?: LogWatch[]): ResolvedWatch[] {
		...
		return input.map((watch, index) => {
			...
			try {
				...
				throw new Error(`Invalid log watch pattern at logWatches[${index}]: ${message}`);
			}

			const stream = watch.stream ?? "both";
			if (stream !== "stdout" && stream !== "stderr" && stream !== "both") {
				throw new Error(`Invalid logWatches[${index}].stream: ${stream}. Expected stdout, stderr, or both`);
			}
	...
	private extractCompleteLines(managed: ManagedProcess, source: "stdout" | "stderr", data: Buffer): string[] {
		const chunk = data.toString();
		const pending = source === "stdout" ? managed.stdoutPendingLine : managed.stderrPendingLine;
	...
	private flushPendingLines(managed: ManagedProcess): void {
		if (managed.stdoutPendingLine) {
			try {
				appendFileSync(managed.combinedFile, `1:${managed.stdoutPendingLine}\n`);
			} catch {
				// Ignore
			}
		...
		if (managed.stderrPendingLine) {
			try {
				appendFileSync(managed.combinedFile, `2:${managed.stderrPendingLine}\n`);
			} catch {
				// Ignore
			}
	...
	private matchWatches(managed: ManagedProcess, source: "stdout" | "stderr", lines: string[]): void {
...
export type { ProcessInfo, ProcessStatus, ManagerEvent, KillResult, WriteResult };
