...
export async function createCheckpoint(opts: CreateCheckpointOpts): Promise<CheckpointData> {
	...
	try {
		...
		const { filtered, allUntracked, skippedLargeFiles, skippedLargeDirs } = await getFilesToAdd(root);
...
export async function loadAllCheckpoints(root: string, sessionId?: string): Promise<CheckpointData[]> {
	...
	return results.filter((cp): cp is CheckpointData => cp !== null && (!sessionId || cp.sessionId === sessionId));
}
...
export function findClosestCheckpoint(checkpoints: CheckpointData[], targetTs: number): CheckpointData | undefined {
